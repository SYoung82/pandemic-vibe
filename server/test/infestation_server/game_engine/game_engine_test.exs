defmodule InfestationServer.GameEngine.GameEngineTest do
  use InfestationServer.DataCase, async: true

  alias InfestationServer.{Games, Repo}
  alias InfestationServer.GameEngine.GameEngine
  import InfestationServer.GamesFixtures
  ...
  describe "initialize_game/1" do
    test "successfully initializes game with 2 players" do
      ...
      assert Enum.all?(players_with_planet, fn p ->
               p.current_planet_id == atlanta.id
             end)
    ...
    @tag :skip
    test "performs initial infection with 9 planets" do
      game = game_fixture_with_players(2)
      ...
      total_cubes =
        Enum.reduce(city_infections, 0, fn {_planet, colors}, acc ->
