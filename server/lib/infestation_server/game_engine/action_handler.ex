defmodule InfestationServer.GameEngine.ActionHandler do
  @moduledoc """
  Validates and processes player actions.
  """

  alias InfestationServer.{Repo, Games}
  alias InfestationServer.Games.Player
  alias InfestationServer.GameEngine.DeckManager

  @doc """
  Moves a player to an adjacent planet.
  """
  def move_player(player_id, destination_planet_name) do
    player = Games.get_player!(player_id) |> Repo.preload(:current_planet)

    with :ok <- validate_has_actions(player),
         {:ok, destination} <- get_planet(destination_planet_name),
  ...
  @doc """
  Treats disease in current planet (removes 1 cube, or all if Medic).
  """
  ...
  @doc """
  Builds a research station in current planet.
  """
  ...
  def discover_cure(player_id, color, card_ids) do
    ...
    with :ok <- validate_has_actions(player),
      ...
      Enum.each(card_ids, fn card_id ->
        card = Repo.get!(InfestationServer.Games.Card, card_id)
  ...
  @doc """
  Shares knowledge: gives a planet card to another player in same planet.
  """
  def share_knowledge(giver_id, receiver_id, card_id) do
    giver = Games.get_player!(giver_id) |> Repo.preload(:current_planet)
    receiver = Games.get_player!(receiver_id) |> Repo.preload(:current_planet)
    card = Repo.get!(InfestationServer.Games.Card, card_id) |> Repo.preload(:planet)

    with :ok <- validate_has_actions(giver),
         :ok <- validate_same_planet(giver, receiver),
         :ok <- validate_card_matches_planet(card, giver.current_planet) do
      card
  ...
  defp validate_adjacent_or_direct_flight(player, destination) do
    ...
    if !current_planet do
      {:error, :no_current_planet}
    else
      # Check if destination is adjacent to current planet
      connected_planets = Games.get_connected_planets(current_planet.id)
      is_adjacent = Enum.any?(connected_planets, &(&1.id == destination.id))

      if is_adjacent do
        :ok
      else
        # TODO: Check for direct flight (player has destination planet card)
        # TODO: Check for charter flight (player has current planet card)
        # TODO: Check for shuttle flight (both planets have research stations)
        {:error, :not_adjacent}
      end
    end
  end

  defp validate_disease_present(game_state, planet_name, color) do
    city_infections = game_state.state_data["city_infections"] || %{}
    cube_count = get_in(city_infections, [planet_name, color]) || 0
  ...
  defp validate_no_station_exists(game_state, planet_name) do
    stations = game_state.state_data["research_stations"] || []

    if planet_name in stations do
  ...
  defp validate_at_research_station(game_state, planet_name) do
    stations = game_state.state_data["research_stations"] || []
  ...
  defp validate_cure_cards(player_id, card_ids, color, needed_count) do
    ...
    cond do
      ...
      Enum.all?(selected_cards, fn card -> card.planet.color == color end) ->
  ...
  defp validate_card_matches_planet(card, planet) do
    if card.planet_id == planet.id do
      :ok
    else
      {:error, :card_does_not_match_planet}
    end
  end

  defp get_planet(name) do
    case Games.get_planet_by_name(name) do
      nil -> {:error, :planet_not_found}
      planet -> {:ok, planet}
    end
  end
  ...
  defp remove_infestation_markers(game_id, planet, color, count) do
    ...
    current_count = get_in(city_infections, [planet.name, color]) || 0
    ...
    updated_city_infections =
      if new_count == 0 do
        ...
        |> Map.get(planet.name, %{})
        |> Map.delete(color)
        |> then(&Map.put(city_infections, planet.name, &1))
      else
        put_in(city_infections, [Access.key(planet.name, %{}), color], new_count)
      end
