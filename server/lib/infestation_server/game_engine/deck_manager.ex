defmodule InfestationServer.GameEngine.DeckManager do
  @moduledoc """
  ...
  def initialize_decks(game_id, difficulty) do
    planets = Games.list_planets()

    # Create player deck with planet cards
    player_cards = create_player_deck(game_id, planets)
    ...
    create_infection_deck(game_id, planets)

    {:ok, player_deck_with_epidemics}
  end

  defp create_player_deck(game_id, planets) do
    planets
    |> Enum.with_index()
  ...
  defp create_infection_deck(game_id, planets) do
    planets
    |> Enum.with_index()
    |> Enum.map(fn {planet, index} ->
      %{
