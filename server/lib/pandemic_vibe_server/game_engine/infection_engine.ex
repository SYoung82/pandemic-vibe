defmodule PandemicVibeServer.GameEngine.InfectionEngine do
  @moduledoc """
  ...
  @doc """
  Performs initial infection setup (3-2-1 cubes on first 9 cities).
  """
  def initial_infection(game_id) do
    ...
    |> Enum.each(fn {card, index} ->
      ...
      infect_city(game_id, card.city, cube_count)
  ...
  @doc """
  Infects a city with disease cubes.
  """
  def infect_city(game_id, city, cube_count \\ 1) do
    ...
    if state == nil do
      ...
      current_count = Map.get(city_infections, city.name, %{}) |> Map.get(city.color, 0)
      new_count = current_count + cube_count

      cond do
        new_count > @max_cubes_per_city ->
          # Outbreak!
          trigger_outbreak(game_id, city, state)
        ...
        true ->
          ...
          updated_city_infections =
            put_in(
              city_infections,
              [Access.key(city.name, %{}), city.color],
              new_count
            )

          updated_disease_cubes = Map.update!(disease_cubes, city.color, &(&1 - cube_count))
  ...
  @doc """
  Draws infection cards and infects cities.
  """
  def draw_infection_cards(game_id, count) do
    ...
    Enum.each(infection_cards, fn card ->
      infect_city(game_id, card.city, 1)
      DeckManager.discard_card(card, "infection_discard")
    end)
  ...
  def handle_epidemic(game_id) do
    ...
    if bottom_card do
      infect_city(game_id, bottom_card.city, 3)
  ...
  defp trigger_outbreak(game_id, city, _state) do
    ...
    # TODO: Implement chain reaction to neighboring cities
